...
class LLMService:
    ...
    def calendar_events_from_prompt(self, prompt: str, iana_timezone: str) -> list[CalendarEvent]:
        llm_prompt = f"""
        ...
        start and end should be in compact ISO 8601 format: YYYYMMDDTHHMMSS
        ...
        And for reference it is currently {format_nice_date(datetime.now(ZoneInfo(iana_timezone)))}, {format_nice_time(datetime.now(ZoneInfo(iana_timezone)))}
        """
        ...
        return self._convert_json_to_calendar_events(event_data, iana_timezone)
    ...
    def _convert_json_to_calendar_events(self, json_data: list[dict], iana_timezone: str) -> list[CalendarEvent]:
        output_list = []
        for element in json_data:
            output_list.append(self._convert_json_to_calendar_event(element, iana_timezone))
        return output_list

    def _convert_json_to_calendar_event(self, json_data: dict, iana_timezone: str) -> CalendarEvent:
        json_data["uid"] = f"{uuid.uuid4()}@{DOMAIN_NAME}"
        json_data["dtstamp"] = datetime.now(ZoneInfo(iana_timezone))
        ...
        for key in ("start", "end"):
            if isinstance(json_data.get(key), str):
                json_data["dt" + key] = (
                    datetime.strptime(json_data[key], "%Y%m%dT%H%M%S")
                    .replace(tzinfo=ZoneInfo(iana_timezone))  # add timezone info
                    .astimezone(ZoneInfo("UTC"))  # convert to UTC
                )
                del json_data[key]
